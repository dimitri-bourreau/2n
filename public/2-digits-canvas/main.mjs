function writeTwo({ x, y }, color, size, context) {
  context.font = `${size}em Arial`;
  context.fillStyle = color;
  context.fillText('2', x, y);
}

function animateCanvas(linesOfTwos, context, animation, { width, height }) {
...
function defineAnimation(lines) {
  const animation = [];
...
function defineHowManyColumns(width) {
  return Math.floor(width / 50);
}

function defineHowManyLines(height) {
  return Math.floor(height / 50);
}

function getNiceColorPalette() {
  const palettes = [
...
function initializeTwos(columns, lines, context, { x: xSpace, y: ySpace }) {
